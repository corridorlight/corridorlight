...
class TrafficSignal:
    ...
    def __init__(
        ...
        # congestion override: phase -> served incoming lanes (indices)
        self._sig_lane_pos: List[Optional[int]] = []
        self._phase_served_lanes: List[List[int]] = []
        self._build_phase_served_lanes()

    def _build_phase_served_lanes(self) -> None:
        """Precompute which incoming lanes are served by each green phase."""
        self._sig_lane_pos = []
        self._phase_served_lanes = []
        try:
            lane_pos = {lid: i for i, lid in enumerate(self.lanes)}
        except Exception:
            lane_pos = {}
        try:
        ...
        if not controlled_links:
            ...
            self._phase_served_lanes = [[] for _ in range(int(getattr(self, "num_green_phases", 0)))]
            return
        # controlled_links index aligns with phase.state character index
        for links in controlled_links:
            in_lane = None
            try:
                if links and links[0] and len(links[0]) > 0:
                    in_lane = links[0][0]
            except Exception:
                in_lane = None
            self._sig_lane_pos.append(lane_pos.get(in_lane) if in_lane in lane_pos else None)
        try:
            phases = list(getattr(self, "green_phases", []) or [])
        except Exception:
            phases = []
        for p in phases:
            served: set[int] = set()
            try:
                st = str(getattr(p, "state", ""))
            except Exception:
                st = ""
            for i, ch in enumerate(st):
                if ch not in ("G", "g"):
                    continue
                if i < len(self._sig_lane_pos):
                    lp = self._sig_lane_pos[i]
                    if lp is not None:
                        served.add(int(lp))
            self._phase_served_lanes.append(sorted(served))
    ...
    def compute_reward(self) -> Union[float, np.ndarray]:
        ...
        if self.reward_scale != 1.0:
            self.last_reward = self.last_reward * self.reward_scale
    ...
    def _queue_reward(self):
        # Normalize queue to reduce reward magnitude
        return -self.get_total_queued() / 50.0
    ...
    def _waiting_time_reward(self):
        """Negative total waiting time (not diff)."""
        ts_wait = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0
        return -ts_wait


    def _waiting_time_reduction_reward(self):
        """Non-negative reduction of total waiting time."""
        ts_wait = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0
        reward = max(self.last_ts_waiting_time - ts_wait, 0.0)
        self.last_ts_waiting_time = ts_wait
        return reward
    ...
    reward_fns = {
        ...
        "waiting-time-reduction": _waiting_time_reduction_reward,
