...
class SumoEnvironment(gym.Env):
    ...
    Args:
        ...
        reward_scale (float): Scale factor applied to rewards. Default: 1.0
    ...
    def _start_simulation(self):
        ...
        # libsumo uses a singleton connection: within the same Python process, doing start/close multiple times
        # (e.g., hyperparameter search) may occasionally trigger "A network was not yet constructed.".
        # Here we: try to close before starting, do an immediate sanity check after starting, and if it fails,
        # restart once and provide clearer diagnostics.
        def _do_start():
            if LIBSUMO:
                try:
                    traci.close()
        ...
        def _sanity_check_network() -> None:
            # Trigger a getTime(); if the network wasn't constructed, it will raise immediately.
            _ = self.sumo.simulation.getTime()

        _do_start()
        try:
            _sanity_check_network()
        except Exception as e:
            msg = str(e).lower()
            if "network was not yet constructed" in msg:
                # retry once
                try:
                    traci.close()
                except Exception:
    ...
    def _apply_actions(self, actions):
        ...
        if self.single_agent:
            if self.traffic_signals[self.ts_ids[0]].time_to_act:
                ts_obj = self.traffic_signals[self.ts_ids[0]]
                ts_obj.set_next_phase(int(actions))
        else:
            for ts, action in actions.items():
                if self.traffic_signals[ts].time_to_act:
                    ts_obj = self.traffic_signals[ts]
                    ts_obj.set_next_phase(int(action))
